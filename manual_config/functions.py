...
def create_ingest_manager(account_identifier, user_login_name, db_name, schema_name, pipe_name, private_key_text):
    try:
...
def ingest_files(ingest_manager, file_paths, container_client):
    try:
        for path in file_paths:
            ...
            sorted_blobs = sorted(blob_list, key=lambda blob: blob.last_modified, reverse=True)
            ...
            if resp['responseCode'] == 200:
                ...
                logging.error(f"Ingestion of file {latest_blob.name} failed with response code: {resp['responseCode']}")
